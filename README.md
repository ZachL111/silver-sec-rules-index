...
silver-sec-rules-index is a PHP project for security tooling. It focuses on this technical goal: Implement a PHP security tooling project for rules incremental indexing, using append-only fixtures and checkpoint recovery checks.
...
The core module exposes a small scoring API. Inputs are simple numeric signals: demand, capacity, latency, risk, and weight. The score uses a threshold of 150, risk penalty 6, latency penalty 2, and weight bonus 5. Tests exercise the public API against the fixture cases in `fixtures/cases.csv`.
