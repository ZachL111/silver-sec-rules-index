...
`silver-sec-rules-index` is centered on security tooling work in PHP. The technical target is: Implement a PHP security tooling project for rules incremental indexing, using append-only fixtures and checkpoint recovery checks.
...
The score is deterministic. The current threshold is 150. Risk has a penalty of 6, latency has a penalty of 2, and weight contributes a bonus of 5.
