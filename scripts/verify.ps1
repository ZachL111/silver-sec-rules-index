...
powershell -NoProfile -ExecutionPolicy Bypass -File scripts/check-silver-sec-rules-detail.ps1

php tests/test_domain_review.php

powershell -NoProfile -ExecutionPolicy Bypass -File scripts/check-silver-sec-rules-properties.ps1
if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }

powershell -NoProfile -ExecutionPolicy Bypass -File scripts/check-silver-sec-rules-golden.ps1
if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }

powershell -NoProfile -ExecutionPolicy Bypass -File scripts/check-silver_sec_rules_index-casebook.ps1
if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }
